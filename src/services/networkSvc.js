...
export default {
  async init() {
    ...
    // For a purely local app we only rely on navigator.onLine and the
    // online/offline events — no external connectivity probe.
    const checkOffline = () => {
  ...
  async getServerConf() {
    // No backend server: seed the local configuration directly.
    if (!isConfLoaded) {
      isConfLoaded = true;
      await store.dispatch('data/setServerConf', { allowSponsorship: false });
    }
  },
  isWindowFocused() {
